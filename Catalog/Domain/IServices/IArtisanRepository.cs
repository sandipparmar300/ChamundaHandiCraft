...
{
    Task<PagedResult<ArtisanGridItem>> GetGridAsync(DataTableRequest request, CancellationToken ct = default);
    Task<ArtisanSaveRequest?> GetByIdAsync(int id, CancellationToken ct = default);
    Task<int> SaveAsync(ArtisanSaveRequest request, int? adminUserId = null, CancellationToken ct = default);
